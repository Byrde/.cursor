...
1. **Read Current State** — Backlog; per-task `docs/testability/` files from task/Notes (and README index); legacy `docs/testability.md` if needed. Backlog: GitHub Project (v2) **#4** in `Byrde/.cursor` (priority field `Priority`, status field `Status`, size field `Size`). Installed defaults: pre-dev `/architect-2` **optional**; post-dev `/architect-2` **optional**; adversarial testing **optional**. Target: prompt task or first `Ready to Test`. Read acceptance criteria—then what they omit. Rapid fix: original issue, expected fix, regression hotspots.
